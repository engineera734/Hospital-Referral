...
import {
  Calculator, TrendingUp, Wallet, FileText, Search, Bell,
  Settings, ChevronDown
} from "lucide-react";

const navItems = [
  { key: "overview", label: "نظرة عامة", icon: TrendingUp },
  { key: "profits", label: "مستحقات الأطباء", icon: Wallet },
  { key: "reports", label: "التقارير المالية", icon: FileText },
];
    ...
    <div className="min-h-screen bg-dark-bg">
      <aside className="fixed right-0 top-0 z-50 h-full w-64 border-l border-border bg-sidebar">
        <div className="border-b border-border p-6">
          ...
          <h1 className="bg-gradient-to-r from-emerald-400 to-teal-400 bg-clip-text text-center text-xl font-bold text-transparent">
            المحاسبة
          </h1>
          <p className="mt-1 text-center text-xs text-gray-500">لوحة التحكم المالية</p>
        </div>
          ...
          {navItems.map((item) => (
            <a
              ...
              className={`flex items-center gap-3 px-6 py-3 transition-all duration-200 ${
                active === item.key
                  ? "border-r-2 border-emerald-500 bg-gray-800 text-white"
      ...
      <main className="mr-64 p-6">
        <header className="mb-8 flex items-center justify-between">
          <div className="relative">
            ...
            <input
              type="text"
              placeholder="بحث عن طبيب أو عملية..."
              className="w-96 rounded-xl border border-border bg-card-bg py-2.5 pl-4 pr-10 text-right text-gray-300 transition-all focus:border-emerald-500 focus:outline-none"
            />
          </div>
          ...
          <div className="flex items-center gap-5">
            ...
            <div className="flex cursor-pointer items-center gap-2" onClick={() => setProfileOpen(true)}>
              <div className="flex h-9 w-9 items-center justify-center overflow-hidden rounded-full bg-gradient-to-br from-emerald-500 to-teal-600 ring-2 ring-emerald-500/30">
                {avatarUrl ? (
              ...
              <div className="hidden text-right md:block">
                ...
                <p className="text-xs text-gray-500">محاسب</p>
              </div>
